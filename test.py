...
for idx, img_name in enumerate(test_images):
    ...
    if preds.masks is not None:
        for box, md in zip(preds.boxes, preds.masks):
            cls  = int(box.cls[0])
...
with open(parsel_csv, "w", newline="") as f:
    w = csv.writer(f)
    w.writerow(["tarih","goruntu_sayisi","toplam_alan_m2","crop_alan_m2","weed_alan_m2","weed_yogunluk_pct","ort_weed_sayisi"])
...
for idx, r in enumerate(results):
    ...
    if img_dets:
        ...
        pdf.cell(0, 3.5, f"  {'Sinif':<6} {'X(px)':>6} {'Y(px)':>6} {'X(m)':>7} {'Y(m)':>7} {'Alan_m2':>9} {'Guven':>6}", new_x="LMARGIN", new_y="NEXT")
        pdf.cell(0, 3.5, "  "+"-"*52, new_x="LMARGIN", new_y="NEXT")
        for det in img_dets[:15]:
            pdf.cell(0, 3.5, f"  {det['class']:<6} {det['cx_px']:>6} {det['cy_px']:>6} {det['local_x_m']:>7.3f} {det['local_y_m']:>7.3f} {det['area_m2']:>9.5f} {det['confidence']:>6.2f}", new_x="LMARGIN", new_y="NEXT")
        if len(img_dets) > 15:
...
pdf.add_page()
pdf.baslik("8. Tum Koordinat Tablosu (Ilk 80 Tespit)")
pdf.set_font("Courier", "", 5.5)
ch = f"{'Goruntu':<22} {'Sinif':>5} {'X_px':>5} {'Y_px':>5} {'X_m':>6} {'Y_m':>6} {'Alan_m2':>9} {'Conf':>5}"
pdf.cell(0, 3.5, ch, new_x="LMARGIN", new_y="NEXT")
pdf.cell(0, 3.5, "-"*len(ch), new_x="LMARGIN", new_y="NEXT")
for d in all_dets[:80]:
    pdf.cell(0, 3.5, f"{d['image']:<22} {d['class']:>5} {d['cx_px']:>5} {d['cy_px']:>5} {d['local_x_m']:>6.3f} {d['local_y_m']:>6.3f} {d['area_m2']:>9.5f} {d['confidence']:>5.2f}", new_x="LMARGIN", new_y="NEXT")
