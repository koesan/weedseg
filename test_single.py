...
if detections:
    ...
    pdf.cell(0, 4, f"  {'#':>3} {'Sinif':<6} {'X(px)':>6} {'Y(px)':>6} {'X(m)':>7} {'Y(m)':>7} {'Alan_m2':>10} {'Guven':>6}", new_x="LMARGIN", new_y="NEXT")
    pdf.cell(0, 4, "  " + "-" * 58, new_x="LMARGIN", new_y="NEXT")
    for i, d in enumerate(detections):
        pdf.cell(0, 4, f"  {i+1:>3} {d['sinif']:<6} {d['cx_px']:>6} {d['cy_px']:>6} {d['x_m']:>7.3f} {d['y_m']:>7.3f} {d['alan_m2']:>10.5f} {d['guven']:>6.2f}", new_x="LMARGIN", new_y="NEXT")
        if pdf.get_y() > 270:
            pdf.add_page()
            pdf.set_font("Courier", "", 6.5)
